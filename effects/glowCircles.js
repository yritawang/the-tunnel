...
const GLOW_INTENSITY = [
  ...
  { count: 24, maxRadius: 380, speed: 1.8, pulseRate: 0.11 },
];
...
function startGlowCirclesEffect(sectionIndex, p5CanvasDiv, sectionMs) {
  ...
  const GROW_OUT_MS = 2500;             
  const GROW_OUT_START = (sectionMs || 15000) - GROW_OUT_MS; 
  ...
  return new p5(sk => {
    ...
    function drawSmoothGlow(x, y, radius, col, alpha) {
      ...
      grad.addColorStop(0.75, `rgba(${r},${g},${b},${alpha * 0.12})`);
